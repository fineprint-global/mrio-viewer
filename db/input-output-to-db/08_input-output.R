...
for(t in nrow(allocation)){
  ...
  query <- sprintf('SELECT DISTINCT year FROM "%s" WHERE allocation = %s;', 
                   "final_demand",
                   allocation[t,]$id)
  ...
  for(year in year_range){
    ...
    insert_data <- insert_data %>% 
      ...
      dplyr::mutate(from_region = rep(region$id, each = 130, times = 130*192)) %>% # each for 130 products, 130 products * 192 countries
      dplyr::mutate(from_product = rep(product$id, times = 130*192^2)) %>% # 130 products * 192 countries * 192 countries
      dplyr::mutate(to_region = rep(region$id, each = 130*192*130)) %>% # first, there is always region 1, then region 2 etc.
      dplyr::mutate(to_product = rep(product$id, each = 130*192, times = 192)) %>% # the first column is to_product 1 and to_region 1 130*192 times
